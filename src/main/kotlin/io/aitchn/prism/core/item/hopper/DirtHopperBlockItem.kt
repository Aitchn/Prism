...
object DirtHopperBlockItem: PrismBlockItem() {
    override val block: PrismBlock = object: PrismBlock() {
        override fun onInventoryMoveItem(event: InventoryMoveItemEvent, block: Block) {
            Bukkit.getScheduler().runTask(Prism.instance, Runnable {
                ...
                hopper.transferCooldown = 19
                hopper.update()
            })
        }
    }
    override val id: Key = PrismUtil.key("dirt_hopper")
    override val name: Component = Component.translatable("item.prism.dirt_hopper", "Dirt Hopper")
    ...
    override val recipes: List<Recipe> = listOf(
        ShapedRecipe(id.conversion(), build()).apply {
            shape("DDD", "DHD", "DDD")
            setIngredient('D', ItemStack(Material.DIRT))
