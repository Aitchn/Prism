...
object IronHopperBlockItem: PrismBlockItem() {
    override val block: PrismBlock = object: PrismBlock() {
        override fun onInventoryMoveItem(event: InventoryMoveItemEvent, block: Block) {
            Bukkit.getScheduler().runTask(Prism.instance, Runnable {
                ...
                hopper.transferCooldown = 5
                hopper.update()
            })
        }
    }
    override val id: Key = PrismUtil.key("iron_hopper")
    override val name: Component = Component.translatable("item.prism.iron_hopper", "Iron Hopper")
    ...
    override val recipes: List<Recipe> = listOf(
        ShapedRecipe(id.conversion(), build()).apply {
            ...
            setIngredient('H', CopperHopperBlockItem.build())
            group = id.asString()
        }
    )
}